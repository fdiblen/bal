...
git branch -M main
git remote add origin https://github.com/fdiblen/bal
```

Go to
[https://github.com/fdiblen?tab=repositories](https://github.com/fdiblen?tab=repositories)
and create a new repository named bal as an empty repository, then:
...
- Sonarcloud must be configured for the analysis to work
  ...
  4. set up repository
  5. go to [new code definition administration page](https://sonarcloud.io/project/new_code?id=fdiblen_bal) and select `Number of days` option
- The analysis will be run by [GitHub Action workflow](.github/workflows/sonarcloud.yml)
- To be able to run the analysis, a token must be created at [Sonarcloud account](https://sonarcloud.io/account/security/) and this token must be added as `SONAR_TOKEN` to [secrets on GitHub](https://github.com/fdiblen/bal/settings/secrets/actions)

## Package version number

- We recommend using [semantic versioning](https://guide.esciencecenter.nl/#/best_practices/releases?id=semantic-versioning).
- For convenience, the package version is stored in a single place: `bal/.bumpversion.cfg`.
...
save it as a secret called `PYPI_TOKEN` on [Settings page](https://github.com/fdiblen/bal/settings/secrets/actions)

[Creating a release](https://github.com/fdiblen/bal/releases/new) on GitHub will trigger a [GitHub action workflow](.github/workflows/publish.yml) to publish the release on PyPI for you.
