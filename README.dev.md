...
# install bal as an editable package
...
# recursively check import style for the bal module only
isort --recursive --check-only bal

# recursively check import style for the bal module only and show
# any proposed changes as a diff
isort --recursive --check-only --diff bal

# recursively fix import style for the bal module only
isort --recursive bal
```
...
cd $(mktemp -d --tmpdir bal.XXXXXX)

# fresh git clone ensures the release has the state of origin/main branch
git clone https://github.com/fdiblen/bal .
...
Visit
[https://test.pypi.org/project/bal](https://test.pypi.org/project/bal)
and verify that your package was uploaded successfully. Keep the terminal open, we'll need it later.

In a new terminal, without an activated virtual environment or an env directory:

```shell
cd $(mktemp -d --tmpdir bal-test.XXXXXX)
...
--extra-index-url https://pypi.org/simple bal
```
